			...
			<? if ($arParams['PRODUCT_DISPLAY_MODE'] === 'Y' && $haveOffers && !empty($item['OFFERS_PROP'])): ?>
				...
				?>
					<div data-entity="sku-block" id="<?= $itemIds['PROP_DIV'] ?>">
						<ul class="item__sku-line-list" data-entity="sku-line-block">
							<? foreach ($skuProperty['VALUES'] as $value):
								if (!isset($item['SKU_TREE_VALUES'][$propertyId][$value['ID']]))
									continue;
								$value['NAME'] = htmlspecialcharsbx($value['NAME']);
							?>
								<li class="item__sku-line-item" title="<?= $value['NAME'] ?>"
									data-treevalue="<?= $propertyId ?>_<?= $value['ID'] ?>" data-onevalue="<?= $value['ID'] ?>">
									<button type="button" class="item__sku-btn"><?= $value['NAME'] ?></button>
								</li>
							<? endforeach; ?>
						</ul>
					</div>
