...
<div class="item">
	<div class="item__gallery-wrapper" >
		...
		<div class="item__action">
			<button class="item__button item__action-btn item__action-btn--quickview" type="button" aria-label="Быстрый просмотр">
				<svg class="item__action-btn-icon" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
				...
				<span  class="item__action-btn-text">Быстрый просмотр</span>
			</button>
			<button class="item__button item__action-btn item__action-btn--oneclickbuy" type="button" aria-label="Купить в один клик">
				<svg class="item__action-btn-icon" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
				...
				<span class="item__action-btn-text">Купить в один клик</span>
			</button>
		</div>
	</div>
	...
	<div class="item__purchase">
		<div class="item__price" id="<?= $itemIds['BASKET_ACTIONS'] ?>">
			<?/*
			if ($showSubscribe) {
			...
			*/ ?>
			...
			<!-- <span><?= $arParams['SKU_PROPS']['RAZMER']["NAME"] ?> - <?= $arParams['SKU_PROPS']['RAZMER']["VALUES"][940]["NAME"] ?></span> -->
				<? if (!empty($price)): ?>
			<div class="item__price-block" data-entity="price-block">
		...
		<div class="item__quantity-block" data-entity="quantity-block">
			<div class="item__counter">
				<button class="item__button item__quantity-btn item__quantity-btn--minus" aria-label="Убрать единицу товара" id="<?= $itemIds['QUANTITY_DOWN'] ?>">-</button>
				<input class="item__quantity-field" id="<?= $itemIds['QUANTITY'] ?>" type="number"
					name="<?= $arParams['PRODUCT_QUANTITY_VARIABLE'] ?>"
					value="<?= $measureRatio ?>" tabindex="-1">
				<button class="item__button item__quantity-btn item__quantity-btn--plus" aria-label="Добавить единицу товара" id="<?= $itemIds['QUANTITY_UP'] ?>">+</button>
			</div>
			<button class="main-btn item__button item__add-to-cart-btn" aria-label="Добавить товар в корзину" id="<?= $itemIds['BUY_LINK'] ?>">
				В корзину
			</button>
		</div>
	</div>
		...
		<div class="item__sku" data-entity="sku-block" id="<?= $itemIds['PROP_DIV'] ?>">
			<div class="item__sku-list" data-entity="sku-line-block">
				...
				foreach ($skuProperty['VALUES'] as $value):
					...
					$index++;
				?>
					<button class="item__sku-list-item <?= $index === 1 ? 'selected' : '' ?>"
