(function (t) {
	...
	t.JCCatalogItem = function (t) {
		...
		if (typeof t === "object") {
				...
				case 7:
					if (t.PRODUCT && typeof t.PRODUCT === "object") {
						...
						this.currentQuantityRangeSelected =
							t.PRODUCT.ITEM_QUANTITY_RANGE_SELECTED;
						if (this.showQuantity) {
							...
							if (this.product.checkQuantity) {
								this.product.maxQuantity = this.product.isDblQuantity
									? parseFloat(t.PRODUCT.MAX_QUANTITY)
									: parseInt(t.PRODUCT.MAX_QUANTITY, 10);
							}
							this.product.stepQuantity = this.product.isDblQuantity
								? parseFloat(t.PRODUCT.STEP_QUANTITY)
								: parseInt(t.PRODUCT.STEP_QUANTITY, 10);
				...
				if (t.COMPARE && typeof t.COMPARE === "object") {
					...
					if (t.COMPARE.COMPARE_DELETE_URL_TEMPLATE) {
						this.compareData.compareDeleteUrl =
							t.COMPARE.COMPARE_DELETE_URL_TEMPLATE;
					} else {
						this.useCompare = false;
					}
				} else {
			...
			this.isFacebookConversionCustomizeProductEventEnabled =
				t.IS_FACEBOOK_CONVERSION_CUSTOMIZE_PRODUCT_EVENT_ENABLED;
		}
		if (this.errorCode === 0) {
	...
	t.JCCatalogItem.prototype = {
					...
					for (a in this.offers[this.offerNum].TREE) {
						if (this.offers[this.offerNum].TREE.hasOwnProperty(a)) {
							...
							for (o in this.treeProps) {
								if (
									this.treeProps.hasOwnProperty(o) &&
									this.treeProps[o].ID == n
								) {
		...
		rememberProductRecommendation: function () {
			...
			var o = s.join("."),
				h = new Date(
					new Date().getTime() + 1e3 * 3600 * 24 * 365 * 10,
				).toUTCString();
			document.cookie =
				t + "=" + o + "; path=/; expires=" + h + "; domain=" + BX.cookie_domain;
		},
		startQuantityInterval: function () {
			var t = BX.proxy_context;
			var i =
				t.id === this.visual.QUANTITY_DOWN_ID
			...
			if (this.errorCode === 0 && this.showQuantity) {
				if (this.canBuy) {
					...
					if (!isNaN(t)) {
						...
						i =
							Math.floor(
								Math.round((t * this.precisionFactor) / this.stepQuantity) /
									this.precisionFactor,
							) || 1;
		...
		quantitySet: function (t) {
			...
			if (this.errorCode === 0) {
				...
				if (this.isDblQuantity) {
					this.stepQuantity =
						Math.round(parseFloat(s.STEP_QUANTITY) * this.precisionFactor) /
						this.precisionFactor;
				...
				if (this.showQuantity) {
					var a =
						r.ITEM_PRICES.length &&
						r.ITEM_PRICES[r.ITEM_PRICE_SELECTED] &&
						r.ITEM_PRICES[r.ITEM_PRICE_SELECTED].MIN_QUANTITY !=
							this.minQuantity;
					if (this.isDblQuantity) {
						i =
							Math.round(parseFloat(r.STEP_QUANTITY) * this.precisionFactor) /
								this.precisionFactor !==
								this.stepQuantity ||
							a ||
							r.MEASURE !== s.MEASURE ||
							(this.checkQuantity &&
								parseFloat(r.MAX_QUANTITY) > this.maxQuantity &&
								parseFloat(this.obQuantity.value) > this.maxQuantity);
					} else {
						i =
							parseInt(r.STEP_QUANTITY, 10) !== this.stepQuantity ||
							a ||
							r.MEASURE !== s.MEASURE ||
							(this.checkQuantity &&
								parseInt(r.MAX_QUANTITY, 10) > this.maxQuantity &&
		...
		stopSlider: function (t) {
			...
			if (this.slider.progress) {
				...
				this.slider.progress.options.duration =
					(this.slider.options.interval * i) / 200;
		...
		cycleSlider: function (t) {
			...
			if (this.slider.options.interval && !this.slider.paused) {
				if (this.slider.progress) {
					...
					var i = parseInt(this.obPictSliderProgressBar.style.width);
					this.slider.progress.options.duration =
						(this.slider.options.interval * (100 - i)) / 100;
					this.slider.progress.options.start = { width: i * 10 };
		...
		getItemForDirection: function (t, i) {
			var e = this.getItemIndex(i),
				s =
					(t === "prev" && e === 0) ||
					(t === "next" && e == this.slider.items.length - 1);
				...
				if (this.searchOfferPropIndex(s[0], s[1])) {
					...
					if (
						...
						BX.ajax.runAction("sale.facebookconversion.customizeProduct", {
							data: { offerId: this.offers[this.offerNum]["ID"] },
						});
					}
				}
			}
			...
			if (-1 < n) {
				...
				for (r = n + 1; r < this.treeProps.length; r++) {
					...
					if (
						...
						BX.util.in_array(this.selectedValues[e], o)
					) {
						l[e] = this.selectedValues[e];
					} else {
		...
		changeInfo: function () {
			...
			if (e > -1) {
				if (
					parseInt(this.offers[e].MORE_PHOTO_COUNT) > 1 &&
					this.obPictSlider
				) {
					if (this.obPict) {
						...
						if (this.offers[e].MORE_PHOTO.hasOwnProperty(t)) {
							this.obPictSlider.appendChild(
								BX.create("SPAN", {
									...
									style: {
										backgroundImage:
											"url('" + this.offers[e].MORE_PHOTO[t].SRC + "')",
									},
								}),
							);
						}
					...
					if (this.obPict) {
						if (this.offers[e].PREVIEW_PICTURE) {
							BX.adjust(this.obPict, {
								style: {
									backgroundImage:
										"url('" + this.offers[e].PREVIEW_PICTURE.SRC + "')",
								},
							});
						} else {
							BX.adjust(this.obPict, {
					...
					if (this.secondPict && this.obSecondPict) {
						if (this.offers[e].PREVIEW_PICTURE_SECOND) {
							BX.adjust(this.obSecondPict, {
								style: {
									backgroundImage:
										"url('" + this.offers[e].PREVIEW_PICTURE_SECOND.SRC + "')",
								},
							});
		...
		checkPriceRange: function (t) {
			...
			for (var s in this.currentQuantityRanges) {
				if (this.currentQuantityRanges.hasOwnProperty(s)) {
					...
					if (
						parseInt(t) >= parseInt(i.SORT_FROM) &&
						(i.SORT_TO == "INF" || parseInt(t) <= parseInt(i.SORT_TO))
					) {
						e = true;
			...
			for (var r in this.currentPrices) {
				if (this.currentPrices.hasOwnProperty(r)) {
					if (
						this.currentPrices[r].QUANTITY_HASH ==
						this.currentQuantityRangeSelected
					) {
			...
			if (this.obPrice) {
				if (i) {
					BX.adjust(this.obPrice, {
						html: BX.Currency.currencyFormat(i.RATIO_PRICE, i.CURRENCY, true),
					});
				} else {
		...
		basketResult: function (t) {
			...
			if (r && this.basketAction === "BUY") {
				...
				if (r) {
					...
					switch (this.productType) {
						...
						case 7:
							...
							break;
						case 3:
							s = this.offers[this.offerNum].PREVIEW_PICTURE
				...
				this.obPopupWin.setTitleBar(
					r ? BX.message("TITLE_SUCCESSFUL") : BX.message("TITLE_ERROR"),
				);
				this.obPopupWin.setContent(e);
